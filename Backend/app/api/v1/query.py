...
async def submit_query(request: QueryRequest, db: AsyncSession = Depends(get_db)) -> QueryResponse:
    ...
    try:
        ...
        result = await scheduler.execute({
            "query_id": query_id, "query": request.query,
            "filters": request.filters.model_dump(), "options": request.options.model_dump(),
        })
        ...
        return QueryResponse(query_id=query_id, status="complete",
            processing_time_ms=result.get("processing_time_ms", 0),
            results=result.get("results", []), agent_trace=result.get("agent_trace", {}))

    except (AgentError, VerdictoError) as e:
        record.status = "failed"
        await db.flush()
        log.error("api.query.failed", error=str(e))
        raise HTTPException(status_code=500, detail={"error": e.to_dict()})
    except Exception as e:
        record.status = "failed"
        await db.flush()
        log.error("api.query.unexpected_error", error=str(e))
        raise HTTPException(status_code=500, detail={"error": {"code": "INTERNAL_ERROR", "message": str(e), "query_id": query_id}})
