...
# Extend these once --inspect shows you the real names.
COLUMN_CANDIDATES: dict[str, list[str]] = {
    "case_id":        ["case_id", "id", "doc_id", "filename", "file_name", "name"],
    "citation":       ["citation", "citations", "neutral_citation", "cite", "reporter_citation"],
    "title":          ["title", "case_title", "case_name", "name_of_case"],
    "bench":          ["bench", "judges", "judge", "coram", "bench_strength", "judge_names"],
    "decision_date":  ["decision_date", "judgment_date", "date", "date_of_judgment", "dt"],
    "disposal_nature": ["disposal_nature", "disposal", "outcome", "status", "nature_of_disposal"],
...
def _normalise_case_id(raw: Any) -> str | None:
    """
    Corpus case_ids are the segmented filenames, e.g. '2024_1_1_10_EN'.
    Parquet may carry a path or a filename with an extension; strip both.
    """
    s = _clean(raw)
    if s is None:
        return None
...
async def run(args: argparse.Namespace) -> None:
    ...
    for row in df.itertuples(index=False):
        ...
        cid = _normalise_case_id(rd.get(id_col))
        if not cid:
            continue
    ...
    async with engine.begin() as conn:
        ...
        stmt = sa_text("""
            UPDATE cases SET
                ...
                bench           = COALESCE(:bench,           bench),
                decision_date   = COALESCE(:decision_date,   decision_date),
                disposal_nature = COALESCE(:disposal_nature, disposal_nature),
                petitioner      = COALESCE(:petitioner,      petitioner),
                respondent      = COALESCE(:respondent,      respondent),
                acts_sections   = COALESCE(:acts_sections,   acts_sections),
                title           = CASE WHEN :title IS NOT NULL AND length(:title) > 5
